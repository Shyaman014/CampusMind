    ...
    <div className="min-h-[75vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md p-8 rounded-3xl glass-card border border-indigo-500/30 space-y-6 bg-gradient-to-b from-slate-900 via-slate-900 to-indigo-950/20 shadow-2xl">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400">Enter your campus email to receive a password reset link</p>
        </div>
        ...
        {submitted ? (
          <div className="p-6 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-center space-y-3">
            ...
            <p className="text-xs text-slate-300">
              We have dispatched a password reset link to <span className="font-bold text-indigo-300">{email}</span>. Please check your inbox.
            </p>
            <Link to="/login" className="inline-block text-xs font-bold text-indigo-400 hover:underline pt-2">
              Return to Login
            </Link>
          </div>
        ) : (
