...
import { Plus, MessageSquare, Trash2, EyeOff, ShieldAlert, Star, Search, Edit2, Copy, Archive, Check, X, Code, BookOpen } from 'lucide-react';
  ...
  const renderChatItem = (chat) => {
    ...
    return (
      <div
        ...
        className={`group flex items-center justify-between px-2.5 py-2 rounded-lg text-[13px] cursor-pointer smooth-transition relative ${
          isActive
          ...
          <>
            <div className="flex items-center space-x-2 min-w-0 pl-1">
              {chat.mode === 'coding' ? (
                ...
                <BookOpen className="w-3.5 h-3.5 flex-shrink-0 text-purple-400" />
              ) : (
  ...
  return (
    ...
    >
      <div className="p-3.5 pt-4 space-y-2.5">
        <button
          onClick={onNewChat}
          className="w-full py-2.5 px-3 rounded-xl bg-[#1F1F1F] hover:bg-[#242424] text-white text-sm font-medium flex items-center justify-center space-x-2 border border-[#2A2A2A] smooth-transition shadow-sm"
        >
